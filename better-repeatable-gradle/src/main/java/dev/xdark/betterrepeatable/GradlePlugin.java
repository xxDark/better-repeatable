...
public class GradlePlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		BetterRepeatableExtension extension = project.getExtensions().create("betterRepeatable", BetterRepeatableExtension.class);
		// Inject plugin as dependency to expose Repeatable annotation
		// TODO exclude compiler classes?
		DependencySet dependencies = project.getConfigurations().getByName("compileOnly").getDependencies();
		project.getGradle().addListener(new DependencyResolutionListener() {
			@Override
			public void beforeResolve(ResolvableDependencies unused) {
				try {
					if (extension.isUsePluginAsDependency()) {
						FileCollection fc = project.files(pluginPath);
						dependencies.add(project.getDependencies().create(fc));
					}
				} finally {
